...
setupMiddlewares = (app, config) ->
  ...
  app.use bodyParser.json limit: config.max_body_size or '10mb'
  app.use bodyParser.urlencoded limit: config.max_body_size or '10mb', extended: true
...
setupSession = (app, config) ->
  ...
  redis_client = redis.createClient config.session_redis_port or 6379, config.session_redis_host or '127.0.0.1'
